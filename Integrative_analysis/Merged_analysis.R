...
load("Patient_06_16um.rda")
load("Patient_38_16um.rda")
load("Patient_4_26_16um.rda")
load("Patient_61_16um.rda")
load("Patient_41_67_16um.rda")

Patient_06 <- subset(Patient_06, subset = Block == "NA", invert =TRUE)
Patient_38 <- subset(Patient_38, subset = Block == "NA", invert =TRUE)
Patient_4_26 <- subset(Patient_4_26, subset = Block == "NA", invert =TRUE)
Patient_61 <- subset(Patient_61, subset = Block == "NA", invert =TRUE)
Patient_41_67 <- subset(Patient_41_67, subset = Block == "NA", invert =TRUE)

ST_merged  <- merge(x = Patient_06, y = c(Patient_38, Patient_4_26, Patient_61, Patient_41_67), add.cell.ids = c("A1","A2","B1","B2","C1"), merge.data = TRUE)
...
plot_data <- as.data.frame(table(Idents(ST_integrated), ST_integrated$Patient))
colnames(plot_data) <- c("Cluster", "Patient", "Count")

pdf('cluster_patient_ST_integrated.pdf')
ggplot(plot_data, aes(x = Patient, y = Count, fill = Cluster)) +
...
save(ST_integrated, file="ST_integrated.rda")
